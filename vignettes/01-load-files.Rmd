...
We provide a convinient function that loads a full panicle in R: `read_full_panicle()`. This funcrtion reads a file that stores the panicle architecture (that has extension `.ricepr`) and its counterpart that stores the grain coordinates (that has extension `.ricegr`) and merges them together.
