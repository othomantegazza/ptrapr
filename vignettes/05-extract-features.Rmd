...
As in [the previous vignette](04-visualize-many-panicles-loop-r.html), you can parse many panicles and put them in a list. Check that vignette to for a thorough explanation of these steps.
...
Since, according to the tidy data concept, **each observation must have its own row**, here we use `spread()` to collect all observations concerning one single panicle that are scattered across multiple rows, and to put them in one single row.
